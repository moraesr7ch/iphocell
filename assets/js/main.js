...
// 🔒 SEGURANÇA / UI: Efeito de revelação palavra por palavra no título principal do Hero
document.addEventListener('DOMContentLoaded', () => {
  const headline = document.querySelector('.hero-overlay-title') || document.querySelector('.hero-title');
  const subtext = document.querySelector('.hero-overlay-subtext') || document.querySelector('.hero-subtitle');
  const ctaButton = document.querySelector('.hero-overlay-btn') || document.querySelector('.hero-actions a');

  if (!headline) return;

  // Sobrescreve as animações CSS para permitir controle total do JS (sem alterar styles.css ou index.html)
  ...
  if (subtext) {
    ...
    subtext.style.transition = 'opacity 800ms ease';
  }
  ...
  if (ctaButton) {
    ...
    ctaButton.style.transition = 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)';
  }

  // Divide o texto do título em palavras e envolve em spans individuais
  ...
  words.forEach((word, index) => {
    ...
    span.style.transition = 'opacity 600ms cubic-bezier(0.22, 1, 0.36, 1), transform 600ms cubic-bezier(0.22, 1, 0.36, 1)';
  ...
  // Gatilho sequencial (staggered delay) para as palavras
  ...
  // Fade-in do subtexto após a última palavra terminar de aparecer totalmente
  const lastWordStartDelay = startDelay + ((wordSpans.length - 1) * staggerDelay);
  const subtextDelay = lastWordStartDelay + wordDuration;
  ...
  // Fade-in e slide-up do botão CTA após a finalização do subtexto
  const subtextDuration = 800;
  const ctaDelay = subtextDelay + subtextDuration + 200;
